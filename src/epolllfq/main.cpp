...
#include <eplfqstream.h>

static std::atomic_bool producer_running = false;

void producer_fast(const std::string& threadName, EPLFQStream<MyDataFrame> *stream)
{
...
void consumer_fast(const std::string& threadName, EPLFQStream<MyDataFrame> *stream)
{
    pthread_setname_np(pthread_self(), threadName.c_str());
    size_t lastId = 0;
...
void consumer_slow(const std::string& threadName, EPLFQStream<MyDataFrame> *stream)
{
    pthread_setname_np(pthread_self(), threadName.c_str());
    size_t lastId = 0;
...
void consumer_instant(const std::string& threadName, EPLFQStream<MyDataFrame> *stream)
{
    pthread_setname_np(pthread_self(), threadName.c_str());
...
void transformer_fast(const std::string& threadName, EPLFQStream<MyDataFrame> *recvStream, EPLFQStream<MyDataFrame> *prodStream)
{
    pthread_setname_np(pthread_self(), threadName.c_str());
    size_t count = 1;
...
void run_6threads()
{
    ...
    std::shared_ptr<EPLFQStream<MyDataFrame>> prodStream(new EPLFQStream<MyDataFrame>());
    std::shared_ptr<EPLFQStream<MyDataFrame>> transStream(new EPLFQStream<MyDataFrame>());
...
int main()
{
    run_timed("EPLFQStream-6threads", run_6threads, N_OF_RUNS);

    std::cout << std::endl;
    run_timed("EPLFQStream-overcapacity", run_overcapacity, N_OF_RUNS);
